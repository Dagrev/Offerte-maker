...
describe('ipcKanalen', () => {
  it('bevat alle 56 kanalen uit §6.2 (49 + 3 keuzelijsten, OFM-034, + adres:zoek, OFM-031, + 3 werkzaamheden, OFM-043), uniek, elk met een invoerschema', () => {
    expect(IPC_KANALEN).toHaveLength(56);
    expect(new Set(IPC_KANALEN).size).toBe(56);
  ...
  it('apiNaam maakt camelCase van de kanaalnaam', () => {
    ...
    expect(new Set(IPC_KANALEN.map(apiNaam)).size).toBe(56);
  });
